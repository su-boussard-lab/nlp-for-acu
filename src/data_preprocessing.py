...
def preprocessing(
    ...
    X_test = feature_matrix.loc[TEST_IDS].sort_values(by="PAT_DEID")
    y_train = labels.loc[TRAIN_IDS]
    y_test = labels.loc[TEST_IDS].sort_values(by="PAT_DEID")

    # Drop all the NaN
    # NOTE: in our case, the patients that do not have clinical notes are filled with NaN, so these are dropped from the original dataframe
    ...
    if mode.lower() == "tabular":
        filter = [c for c in X_train.columns if "WORD" not in c]
    elif mode.lower() == "language":
        filter = [c for c in X_train.columns if "WORD" in c]
    elif mode.lower() == "fusion":
        filter = [c for c in X_train.columns]

    X_train = X_train[filter]
    X_test = X_test[filter]

    print(
        f"There are {feature_matrix.shape[0]} patients and {feature_matrix.shape[1]} features in full feature matrix for mode: {mode}."
    )
    print(
        f"There are {X_train.shape[0]} patients and {X_train.shape[1]} features in Train set for mode: {mode}."
    )
    print(
        f"There are {X_test.shape[0]} patients and {X_test.shape[1]} features in Test set for mode: {mode}."
    )
